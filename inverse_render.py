...
optimizer = torch.optim.Adam(
    [w_frequency_offsets, w_phase_shift_offsets], lr=1e-2, weight_decay=1e-4
)
scheduler = torch.optim.lr_scheduler.StepLR(optimizer, 100, gamma=0.75)

for i in range(n_iterations):
    noise_w_frequencies = (
        0.03 * torch.randn_like(w_frequencies) * (n_iterations - i) / n_iterations
    )
    noise_w_phase_shifts = (
        0.03 * torch.randn_like(w_phase_shifts) * (n_iterations - i) / n_iterations
    )
...
writer = skvideo.io.FFmpegWriter(
    os.path.join("debug", output_name), outputdict={"-pix_fmt": "yuv420p", "-crf": "21"}
)
