        ...
        for i, (imgs, _) in enumerate(dataloader):
                ...
                with torch.no_grad():
                    ...
                    for split in range(metadata["batch_split"]):
                        ...
                        g_imgs, g_pos = generator_ddp(
                            subset_z_s, subset_z_a, **metadata
                        )
            ...
            for split in range(metadata["batch_split"]):
                with torch.cuda.amp.autocast():
                    ...
                    if metadata["z_lambda"] > 0 or metadata["pos_lambda"] > 0:
                        latent_penalty = (
                            torch.nn.MSELoss()(g_pred_latent, subset_z_s)
                            + torch.nn.MSELoss()(g_pred_latent, subset_z_a)
                        ) * metadata["z_lambda"]
                        position_penalty = (
                            torch.nn.MSELoss()(g_pred_position, gen_positions)
                            * metadata["pos_lambda"]
            ...
            if rank == 0:
                ...
                if discriminator.step % opt.sample_interval == 0:
                    ...
                    save_image(
                        gen_imgs[:25],
                        os.path.join(opt.output_dir, f"{discriminator.step}_fixed.png"),
                        nrow=5,
                        normalize=True,
                    )
                    ...
                    save_image(
                        gen_imgs[:25],
                        os.path.join(
                            opt.output_dir, f"{discriminator.step}_tilted.png"
                        ),
                        nrow=5,
                        normalize=True,
                    )
                    ...
                    with torch.no_grad():
                        with torch.cuda.amp.autocast():
                            ...
                            gen_imgs = generator_ddp.module.staged_forward(
                                torch.randn_like(fixed_z_s).to(device),
                                torch.randn_like(fixed_z_a).to(device),
                                **copied_metadata,
                            )[0]
                    save_image(
                        gen_imgs[:25],
