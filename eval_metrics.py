...
if __name__ == "__main__":
    ...
    curriculum = curriculums.extract_metadata(
        getattr(curriculums, opt.curriculum), generator.step
    )
    ...
    for img_counter in tqdm(range(opt.num_images)):
        ...
        with torch.no_grad():
            img = generator.staged_forward(
                z, max_batch_size=opt.max_batch_size, **curriculum
            )[0].to(device)
            save_image(
                img,
                os.path.join(opt.output_dir, f"{img_counter:0>5}.jpg"),
                normalize=True,
                range=(-1, 1),
            )
