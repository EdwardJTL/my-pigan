...
def list_img_for_type(type, path):
    for f in os.listdir(path):
        if (
            ...
            and fnmatch.fnmatch(f, "*{}.png".format(type))
        ):
            yield f
...
def make_gif_for_type(type, path, output_path):
    ...
    for filename in files:
        ...
        image = write_step_on_image(
            image, number_re.findall(filename)[0], text_height=20
        )
        images.append(image)
    images[0].save(
        os.path.join(output_path, type + ".gif"),
        save_all=True,
...
def main():
    ...
    files = [
        ...
        if os.path.isfile(os.path.join(abs_input_dir, f))
        and fnmatch.fnmatch(f, "*.png")
    ]
