...
def sample_camera_positions(
    ...
    if mode == "uniform":
        ...
        ) * 2 * horizontal_stddev + horizontal_mean
        phi = (
            torch.rand((n, 1), device=device) - 0.5
        ) * 2 * vertical_stddev + vertical_mean
        ...
        if random.random() < 0.5:
            theta = (
                torch.rand((n, 1), device=device) - 0.5
            ) * 2 * horizontal_stddev * 2 + horizontal_mean
            phi = (
                torch.rand((n, 1), device=device) - 0.5
            ) * 2 * vertical_stddev * 2 + vertical_mean
        else:
            theta = (
                torch.randn((n, 1), device=device) * horizontal_stddev + horizontal_mean
            )
            phi = torch.randn((n, 1), device=device) * vertical_stddev + vertical_mean

    elif mode == "truncated_gaussian":
        theta = (
            truncated_normal_(torch.zeros((n, 1), device=device)) * horizontal_stddev
            + horizontal_mean
        )
        phi = (
            truncated_normal_(torch.zeros((n, 1), device=device)) * vertical_stddev
            + vertical_mean
        )
