...
class FrequencyInterpolator:
    def __init__(self, generator, z1, z2, psi=0.5):
        ...
        self.truncated_frequencies1 = avg_frequencies + psi * (
            raw_frequencies1 - avg_frequencies
        )
        self.truncated_phase_shifts1 = avg_phase_shifts + psi * (
            raw_phase_shifts1 - avg_phase_shifts
        )
        raw_frequencies2, raw_phase_shifts2 = generator.siren.mapping_network(z2)
        self.truncated_frequencies2 = avg_frequencies + psi * (
            raw_frequencies2 - avg_frequencies
        )
        self.truncated_phase_shifts2 = avg_phase_shifts + psi * (
            raw_phase_shifts2 - avg_phase_shifts
        )

    def forward(self, t):
        frequencies = (
            self.truncated_frequencies1 * (1 - t) + self.truncated_frequencies2 * t
        )
        phase_shifts = (
            self.truncated_phase_shifts1 * (1 - t) + self.truncated_phase_shifts2 * t
        )
...
writer = skvideo.io.FFmpegWriter(
    os.path.join(opt.output_dir, output_name),
...
for i, seed in enumerate(opt.seeds):
    ...
    frequencyInterpolator = FrequencyInterpolator(
        generator, z_current, z_next, psi=opt.psi
    )
    ...
    with torch.no_grad():
        for t, pitch, yaw, fov in tqdm(trajectory):
            ...
            frame, depth_map = generator.staged_forward_with_frequencies(
                *frequencyInterpolator.forward(t),
