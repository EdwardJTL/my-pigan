...
def sample_generator(
    ...
    with torch.no_grad():
        ...
        truncated_frequencies = generator.avg_frequencies + psi * (
            raw_frequencies - generator.avg_frequencies
        )
        truncated_phase_shifts = generator.avg_phase_shifts + psi * (
            raw_phase_shifts - generator.avg_phase_shifts
        )

    with torch.no_grad():
        while head < samples.shape[1]:
            coarse_output = generator.siren.forward_with_frequencies_phase_shifts(
...
if __name__ == "__main__":
    ...
    for seed in opt.seeds:
        ...
        voxel_grid = sample_generator(
            generator,
            z,
            cube_length=opt.cube_size,
            voxel_resolution=opt.voxel_resolution,
        )

        os.makedirs(opt.output_dir, exist_ok=True)
        with mrcfile.new_mmap(
            os.path.join(opt.output_dir, f"{seed}.mrc"),
            overwrite=True,
